...
public class BloomeryTileEntity extends MachineTileEntity implements IInventory, IUniversalInventory {
	...
	public IUniversalStack getStackInHOERemoteSlot(int slot){
		//hoe synced
		if(slot==-1 || slot==-2){
			BloomeryData sd = (BloomeryData)this.getClientData();
			if(sd==null){
				return null;
				}
			if(slot==-1){
